...
const CHARS: string = '.PYFGCRLAOEUIDHTNSQJKXBMWVZ_pyfgcrlaoeuidhtnsqjkxbmwvz1234567890'
  .split('').sort().join('');

// d64 char code -> 6-bit index (replaces d64's codeToIndex).
const C2I = new Int8Array(128);
for (let i = 0; i < 64; i++) C2I[CHARS.charCodeAt(i)] = i;

// hex char code -> nibble.
const H2N = new Int8Array(128);
for (let i = 0; i < 10; i++) H2N[48 + i] = i;
for (let i = 0; i < 6; i++) { H2N[97 + i] = 10 + i; H2N[65 + i] = 10 + i; }

// byte -> 2 hex chars.
const B2H: string[] = [];
for (let i = 0; i < 256; i++) B2H[i] = (i + 0x100).toString(16).slice(1);
